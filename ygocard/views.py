...
def card_edit(request, pk):
    card = get_object_or_404(Card, pk=pk)
    c_type = get_object_or_404(CardType)
    m_type = get_object_or_404(MonsterType)
    m_type1 = get_object_or_404(MonsterType1)
    m_type2 = get_object_or_404(MonsterType2)
    m_ability = get_object_or_404(MonsterAbility)
    st_type = get_object_or_404(SpellTrapType)
    c_rarity = get_object_or_404(CardRarity)

    if request.method == "POST":
        form = CardForm(request.POST, request.FILES, instance=card, prefix='crd')
        form_card_type = CardTypeForm(request.POST, instance=c_type, prefix='ctyp')
        form_monster_type = MonsterTypeForm(request.POST, instance=m_type, prefix='mtyp')
        form_monster_type1 = MonsterType1Form(request.POST, instance=m_type1, prefix='mtyp1')
        form_monster_type2 = MonsterType2Form(request.POST, instance=m_type2, prefix='mtyp2')
        form_monster_ability = MonsterAbilityForm(request.POST, instance=m_ability, prefix='mabt')
        form_spell_trap = SpellTrapTypeForm(request.POST, instance=st_type, prefix='stt')
        form_card_rarity = CardRarityForm(request.POST, instance=c_rarity, prefix='crrt')
        ...
        form_card_type = CardTypeForm(instance=c_type, prefix='ctyp')
        form_monster_type = MonsterTypeForm(instance=m_type, prefix='mtyp')
        form_monster_type1 = MonsterType1Form(instance=m_type1, prefix='mtyp1')
        form_monster_type2 = MonsterType2Form(instance=m_type2, prefix='mtyp2')
        form_monster_ability = MonsterAbilityForm(instance=m_ability, prefix='mabt')
        form_spell_trap = SpellTrapTypeForm(instance=st_type, prefix='stt')
        form_card_rarity = CardRarityForm(instance=c_rarity, prefix='crrt')
